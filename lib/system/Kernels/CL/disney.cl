...
float Lambert_GetPdf(DisneyShaderData const* shader_data, float3 wi, float3 wo)
{
	return fabs(wo.y) / PI;
}

float MicrofacetReflectionGGX_D(float roughness, float3 m)
{
...
float MicrofacetReflectionGGX_G1(float roughness, float3 v)
{
...
float MicrofacetReflectionGGX_G(float roughness, float3 wi, float3 wo)
{
	return MicrofacetReflectionGGX_G1(roughness, wi) * MicrofacetReflectionGGX_G1(roughness, wo);
}

float MicrofacetReflectionGGX_GetPdf(DisneyShaderData const* shader_data, float3 wi, float3 wo)
{
	...
	float3 c_spec0 = mix(shader_data->specular * 0.1f * mix(WHITE, c_tint, shader_data->specular_tint), cd_lin, shader_data->metallic);
...
float3 MicrofacetReflectionGGX_Evaluate(DisneyShaderData const* shader_data, float3 wi, float3 wo)
{
	float3 cd_lin = shader_data->base_color;
	float cd_lum = dot(cd_lin, make_float3(0.3f, 0.6f, 0.1f));
	...
	float3 fs = mix(c_spec0, WHITE, SchlickFresnelReflectance(hdotwo));

	return ds * fs * gs;
}

void MicrofacetReflectionGGX_SampleNormal(float roughness, float2 sample, float3* wh)
{
	float r1 = sample.x;
	float r2 = sample.y;
	...
	*wh = normalize(make_float3(cosphi * sintheta, ndotwh, sinphi * sintheta));
}

void MicrofacetReflectionGGX_Aniso_SampleNormal(float roughness, float anisotropy, float2 sample, float3* wh)
{
	float alpha = roughness * roughness;
...
float MicrofacetRefractionGGX_GetPdf(DisneyShaderData const* shader_data, float3 wi, float3 wo)
{
...
float3 MicrofacetRefractionGGX_Evaluate(DisneyShaderData const* shader_data, float3 wi, float3 wo)
{
	...
	return denom > DENOM_EPS ? (ks * (widotwh * wodotwh)  * (etat)* (etat) * MicrofacetReflectionGGX_G(roughness, wi, wo) * MicrofacetReflectionGGX_D(roughness, wh) / denom) : 0.f;
}

float3 MicrofacetRefractionGGX_Sample(DisneyShaderData const* shader_data, float2 sample, float3 wi, float3* wo, float* pdf)
{
	const float3 ks = shader_data->base_color.xyz;
	const float roughness = max(shader_data->roughness, ROUGHNESS_EPS);
	...
	float3 wh;
	MicrofacetReflectionGGX_SampleNormal(shader_data->roughness, sample, &wh);
...
float3 IdealRefract_Evaluate(DisneyShaderData const* shader_data, float3 wi, float3 wo)
{
	return 0.f;
}

float IdealRefract_GetPdf(DisneyShaderData const* shader_data, float3 wi, float3 wo)
{
	return 0.f;
}

float3 IdealRefract_Sample(DisneyShaderData const* shader_data, float2 sample, float3 wi, float3* wo, float* pdf)
{
	const float3 ks = shader_data->base_color.xyz;
...
INLINE float Disney_GetPdf(DisneyShaderData const* shader_data, float3 wi, float3 wo)
{
	...
	else
	{
		...
		float bsdf = mix(cs_w * r_pdf, c_pdf, shader_data->clearcoat);
		float brdf = mix(cs_w * r_pdf + d_pdf * (1.f - cs_w), c_pdf, shader_data->clearcoat);
...
INLINE float3 Disney_Evaluate( DisneyShaderData const* shader_data, float3 wi, float3 wo)
{
	...
	if (shader_data->transmission < 1.0f)
	{
		...
		if (wo.y <= 0.0f)
		{
			float3 d_brdf = base_color * mix(0.f, Diffuse_PennerSkin(ss, wo.y, 0.5f, subsurface_color), subsurface);
			brdf = (1.f / PI) * d_brdf * (1.f - metallic);
		}
		else
		{
			float fd90 = 0.5f + 2 * hdotwo * hdotwo * roughness;
			float fd = mix(1.f, fd90, f_wo) * mix(1.f, fd90, f_wi) * ndotwo;
			float3 d_brdf = mix(fd, Diffuse_PennerSkin(ss, wo.y, 0.5f, subsurface_color), subsurface) * base_color;

			float cd_lum = dot(base_color, make_float3(0.3f, 0.6f, 0.1f));			
			float3 c_tint = cd_lum > 0.f ? (base_color / cd_lum) : WHITE;    
			float3 c_spec0 = mix(specular * 0.1f * mix(WHITE, c_tint, specular_tint), base_color, metallic);    
			...
			brdf = mix((1.f / PI) * (d_brdf + f_sheen) * (1.f - metallic) + gs * fs * ds + clearcoat * gr * fr * dr, 0.f, shader_data->transparency);
		}
	}
...
float3 Disney_Sample(DifferentialGeometry* dg, DisneyShaderData const* shader_data, float2 sample, float3 wi, float3* wo, float* pdf)
{
	...
	else
	{
		...
		if (sample.x < shader_data->clearcoat)
		{
			...
			float3 wh;
			MicrofacetReflectionGGX_SampleNormal(mix(0.001f, 0.1f, shader_data->clearcoat_roughness), sample, &wh);
		...
		else
		{
			...
			if (sample.y < cs_w)
			{
				...
				MicrofacetReflectionGGX_Aniso_SampleNormal(shader_data->roughness, shader_data->anisotropy, sample, &wh);
