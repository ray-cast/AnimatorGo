...
MainWindow::onRecordSignal(bool enable) noexcept
{
	if (behaviour_)
	{
		...
		if (behaviour->isOpen())
		{
			if (enable)
			{
				QString fileName = QFileDialog::getSaveFileName(this, u8"Â¼ÖÆÊÓÆµ", "", tr("HDRi Files (*.h264)"));
				if (!fileName.isEmpty())
				{
...
MainWindow::onImportHdriSignal(bool enable) noexcept
{
	if (behaviour_)
	{
		if (enable)
		{
			...
			if (behaviour->isOpen())
			{
				QString fileName = QFileDialog::getOpenFileName(this, u8"´ò¿ªÍ¼Ïñ", "", tr("HDRi Files (*.hdr)"));
				if (!fileName.isEmpty())
				{
