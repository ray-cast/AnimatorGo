...
namespace octoon
{
	namespace graphics
	{
		...
		bool
		GL33DeviceProperty::setup() noexcept
		{
			assert(_device.lock());
			...
			{
				this->getDevice()->downcast<GL33Device>()->message(e.what());
			}
		...
		void
		GL33DeviceProperty::close() noexcept
		{
		}

		void
		GL33DeviceProperty::setDevice(const GraphicsDevicePtr& device) noexcept
		{
			_device = device;
		}

		GraphicsDevicePtr
		GL33DeviceProperty::getDevice() noexcept
		{
			return _device.lock();
		}
		...
		#if defined(OCTOON_BUILD_PLATFORM_WINDOWS)
		void
		GL33DeviceProperty::setupGLEnvironment(CreateParam& param) noexcept(false)
		{
			WNDCLASSEXA wc;
			...
			param.hwnd = CreateWindowEx(WS_EX_APPWINDOW, "OctoonWin32OpenGLWindow", "GL33", 0, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, wc.hInstance, NULL);
			if (!param.hwnd)
		...
		void
		GL33DeviceProperty::closeGLEnvironment(const CreateParam& param) noexcept
		{
			if (param.context)
			{
		...
		GL33DeviceProperty::setupGLEnvironment(CreateParam& param) noexcept(false)
		{
			...
			if (!param.octx)
			{
				...
				contextAttrs[index++] = (CGLPixelFormatAttribute)kCGLGL33PVersion_3_2_Core;
		...
		void
		GL33DeviceProperty::closeCGLEnvironment(const CreateParam& param) noexcept
		{
			if (param.octx)
			{
		...
		GL33DeviceProperty::getDeviceProperties() const noexcept
		{
