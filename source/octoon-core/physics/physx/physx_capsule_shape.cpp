...
namespace octoon
{
	...
	void
	PhysxCapsuleShape::setCenter(const math::float3& center)
	{
		auto pxLocalPose = px_shape->getLocalPose();
	...
	void
	PhysxCapsuleShape::setRadius(float radius)
	{
	...
	void
	PhysxCapsuleShape::setHeight(float height)
	{
		physx::PxCapsuleGeometry geometry;
		if (px_shape->getCapsuleGeometry(geometry))
		{
	...
	float
	PhysxCapsuleShape::getRadius() const
	{
		physx::PxCapsuleGeometry geometry;
		if (px_shape->getCapsuleGeometry(geometry))
	...
	float
	PhysxCapsuleShape::getHeight() const
	{
		physx::PxCapsuleGeometry geometry;
		if (px_shape->getCapsuleGeometry(geometry))
	...
	physx::PxShape*
	PhysxCapsuleShape::getPxShape()
	{
		return px_shape;
	}

	physx::PxMaterial*
	PhysxCapsuleShape::getPxMaterial()
	{
		return px_material;
	}
}