	...
	bool prepareLayers(const json& j)
	{
		...
		for (auto& layer : j["layers"])
		{
			...
			case LayerTypes::still:
			{
				...
				object = octoon::GamePrefabs::instance()->createSprite(tex);
				object->getComponent<octoon::Transform>()->setScale(octoon::math::float3(tex->getTextureDesc().getWidth(), tex->getTextureDesc().getHeight(), 100));
			}
			break;
