...
class LottieApp
{
	...
	public:
		...
		void prepare(const json& j)
		{
			...
			pos = octoon::math::float3(p[0].get<json::number_float_t>(), p[1].get<json::number_float_t>(), p[2].get<json::number_float_t>());
			scale = octoon::math::float3(s[0].get<json::number_float_t>() / 100.0f, -s[1].get<json::number_float_t>() / 100.0f, s[2].get<json::number_float_t>() / 100.0f);
			rotation = octoon::math::float3(0.0f, 0.0f, -r.get<json::number_float_t>());
			opacity = o.get<json::number_unsigned_t>() / 100.0f;
		}
	};
	...
	bool prepareLayers(const json& j)
	{
		...
		for (auto& layer : j["layers"])
		{
			...
			if (!ks.is_null())
			{
				...
				auto transform = object->getComponent<octoon::Transform>();
				transform->setTranslate(t.pos);
				transform->setScale(t.scale);
				transform->setQuaternion(octoon::math::Quaternion(t.rotation));
			}
