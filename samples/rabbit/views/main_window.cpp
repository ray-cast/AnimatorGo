	...
	MainWindow::onImportSignal() noexcept
	{
		try
		{
			if (behaviour_ && !profile_->timeModule->playing_)
			{
				...
				if (behaviour)
				{
					QString fileName = QFileDialog::getOpenFileName(this, u8"打开项目", "", tr("PMM Files (*.pmm)"));
					if (!fileName.isEmpty())
					{
						try
						{
	...
	MainWindow::open(int w, int h) noexcept
	{
		...
		try
		{
			...
			behaviour->addMessageListener("rabbit:player:finish", [this](const std::any&)
			{
				...
				toolBar_->playButton_->setToolTip(u8"播放");
				...
				recordWindow_->okButton_->setText(u8"开始渲染");
			});
			...
			behaviour->addMessageListener("rabbit:offline", [this](const std::any& enable)
			{
				...
				{
					toolBar_->gpuButton_->setIcon(toolBar_->gpuOnIcon_);
					toolBar_->gpuEnable_ = true;
				}
				else
				{
					toolBar_->gpuButton_->setIcon(toolBar_->gpuIcon_);
