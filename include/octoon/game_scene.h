...
namespace octoon
{
	...
	public:
		...
		std::size_t raycastHit(const math::Raycast& ray, struct RaycastHit& hit, float distance = std::numeric_limits<float>::max(), std::uint32_t layerMask = 0) noexcept;
		std::size_t raycastHit(const math::float3& orgin, const math::float3& end, struct RaycastHit& hit, float distance = std::numeric_limits<float>::max(), std::uint32_t layerMask = 0) noexcept;
