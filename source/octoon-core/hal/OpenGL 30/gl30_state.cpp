...
namespace octoon
{
	namespace hal
	{
		OctoonImplementSubClass(GL30GraphicsState, RenderState, "GL30GraphicsState")
		...
		bool
		GL30GraphicsState::setup(const RenderStateDesc& desc) noexcept
		{
			_stateDesc = desc;
			return true;
		}
		...
		void
		GL30GraphicsState::apply(RenderStateDesc& lastStateDesc) noexcept
		{
			...
			if (_stateDesc.getBlendEnable())
			{
				...
				{
					GLenum sfactorRGB = GL30Types::asBlendFactor(_stateDesc.getBlendSrc());
					GLenum dfactorRGB = GL30Types::asBlendFactor(_stateDesc.getBlendDest());
					GLenum sfactorAlpha = GL30Types::asBlendFactor(_stateDesc.getBlendAlphaSrc());
					GLenum dfactorAlpha = GL30Types::asBlendFactor(_stateDesc.getBlendAlphaDest());
				...
				{
					GLenum modeRGB = GL30Types::asBlendOperation(_stateDesc.getBlendOp());
					GLenum modeAlpha = GL30Types::asBlendOperation(_stateDesc.getBlendAlphaOp());
		...
		GraphicsDevicePtr
		GL30GraphicsState::getDevice() const noexcept
		{
			return _device.lock();
		}
	}
