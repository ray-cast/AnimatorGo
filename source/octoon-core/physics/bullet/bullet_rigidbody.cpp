...
namespace octoon
{
	namespace physics
	{
		...
		BulletRigidbody::setMass(float mass)
		{
			...
			btv3LocalInertia.setX(inertia.x());
			btv3LocalInertia.setY(inertia.y());
			btv3LocalInertia.setZ(inertia.z());

			btCollisionShape* collision = rigidbody_->getCollisionShape();
			if (mass != 0.0f)
				collision->calculateLocalInertia(mass, btv3LocalInertia);

			rigidbody_->setMassProps(mass, btv3LocalInertia);
		}
		...
		bool
		BulletRigidbody::updateMassAndInertia(float density)
		{
			return density;// physx::PxRigidBodyExt::updateMassAndInertia(*rigidbody_, density);
		}
